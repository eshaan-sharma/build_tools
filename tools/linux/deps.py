...
def install_deps():
  ...
  if (nodejs_cur < 10020):
    ...
    print("Reinstall")
    if (base.is_dir("./node_js_setup_12.x")):
      base.delete_dir("./node_js_setup_12.x")
    base.cmd("sudo", ["apt-get", "remove", "--purge", "-y", "nodejs"])
    base.download("https://deb.nodesource.com/setup_12.x", "./node_js_setup_12.x")
    base.cmd('curl -fsSL https://deb.nodesource.com/gpgkey/nodesource.gpg.key | sudo apt-key add -')
    base.cmd("sudo", ["bash", "./node_js_setup_12.x"])
    base.cmd("sudo", ["apt-get", "install", "-y", "nodejs"])
